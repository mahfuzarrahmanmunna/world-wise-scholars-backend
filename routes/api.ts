...
// Fixed route for /scholarship/:id
router.get(
  "/scholarship/:id",
  async (req: express.Request, res: express.Response): Promise<void> => {
    ...
    // FIX: Build conditions dynamically without null
    const conditions: any[] = [];
    conditions.push({ _id: id }); // Check for string ID

    if (ObjectId.isValid(id)) {
      conditions.push({ _id: new ObjectId(id) }); // Check for ObjectId if valid
    }

    const query = conditions.length > 0 ? { $or: conditions } : {};
    const result = await scholarships.findOne(query);
...
router.get("/search/scholarships", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ success: false, message: "Failed to fetch scholarships" });
  }
});
...
router.post("/search/scholarships", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Scholarship search failed" });
  }
});

// Fixed route for /university/:id
...
router.post("/search/universities", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "University search failed" });
  }
});
...
router.delete("/scholarship/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ success: false, message: "Failed to delete scholarship" });
  }
});
...
router.delete("/university/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ success: false, message: "Failed to delete university" });
  }
});
